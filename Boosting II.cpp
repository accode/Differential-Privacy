...
public:
	...
	int buildTree( int x, int l, int r) {
		...
		c =(pow((db)sz[x],d-1.)-1.0)  /  (pow((db)sz[x],(db)d)-1);
		z[x] += c * sum;
		countwithnoise[x] = count[x] + Lap(0, lambda);
		
		z[x] += (pow((db)sz[x],(db)d)-pow((db)sz[x],d-1.))  /  (pow((db)sz[x],(db)d)-1) * countwithnoise[x];
		return d;
	}
	...
	void _dfs(int x,int fa){
		if(x==1) {
			...
			h[x] = z[x];
			db sum=h[fa];
			for(int i=0;i<MAXK;++i) if(nxt[fa][i]!=-1)
				sum -= z[nxt[fa][i]];
			h[x] += sum / sz[fa];
		}
	...
	void CalError( int x, int l, int r){
		...
		if(len >= MAXK) {
			...
			cl = l, cr = cl + len;
			for(int i = 0; i < MAXK; ++ i) {
				CalError(nxt[x][i], cl, cr);
				cl = cr; cr = cl + len; if(i==MAXK-2) cr = r;
			}
		}else{
			for(int i=0;i<MAXK;++i) if(nxt[x][i] != -1){
				CalError(nxt[x][i], l+i, l+i+1);
			}
		}
		if(x == 1) {
			cout <<" brute_force add Lap noise : Error  = " << sqrt(d0) << endl;
...
int main(){
	...
	while(T --) {
		...
		DK.CopyData(A, n);
		DK.setEpsilon(1); DK.setDelta( log(n) / log(MAXK+.0));
		DK.calLambda();
		DK.buildTree(1, 0, n);
		DK._dfs(1, -1);
		//DK.show(1, 0, n);
		DK.CalError(1, 0, n);
		cout << (DK.check(1)?"Correct!" : "No!") << endl;
	}
	return 0;
}
